...
void WeiYan::activateZhuangShi(int discard, int hpLoss) {
    ...
    qDebug() << "【壮誓】已发动：弃" << discard << "张，失" << hpLoss << "血。";
}
...
void WeiYan::useSlash(Player* target) {
    ...
    if (hpSat && handSat) {
        this->heal(1);
        // 现在 CardCategory::OTHER 已经定义了，这里不会报错
        this->drawCard(Card{CardCategory::OTHER, CardType::OTHER, "随机牌", 0});
...
bool WeiYan::activateBeiShui() {
    if (currentState == SUCCESS) {
        ...
        qDebug() << "【背水】发动：先摸一张牌，随后弃置一张牌，【杀】次数+1。";
        return true;
    }
    qDebug() << "【背水】发动失败：状态不满足。";
    return false;
}
...
void WeiYan::endTurn() {
    if (currentState == SUCCESS) {
        ...
        if (cardsUsedThisTurn < zhuangShi_hpLost) {
            ...
            else this->drawCard(Card{CardCategory::OTHER, CardType::OTHER, "随机牌", 0});
